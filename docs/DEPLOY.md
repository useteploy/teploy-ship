...
| `SHIP_MERGE_GATE` | **on wherever `SHIP_CHANGE_CLASS` is** | The boundary park (C1). A change classified `serious` no longer parks before the push: the run publishes a **draft** pull request with all its verification, then parks once on `approve-merge` — approve rebases onto the current default branch, re-runs the suite when the bytes changed, and marks the PR ready (merging it too, on a repo whose `autoMerge` is on); deny closes the PR with the reason. Only a change that **deletes files or touches a schema/migration path** still parks mid-run, before anything is pushed. Set to `0` to restore the old routing for newly-enqueued runs; runs already enqueued replay under whichever routing their input recorded. |
