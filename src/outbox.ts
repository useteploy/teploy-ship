...
export interface Outbox {
  ...
  /** Delivery failed; keep the intent and schedule the next attempt. */
  fail(id: string, now: number): Promise<void>;
}

/** Former abandonment threshold, retained for compatibility and regression tests. */
...
export function notificationId(event: RunNotification): string {
  return `${event.runId}:${event.status}:${event.eventName ?? ""}${event.eventSeq !== undefined ? `:${event.eventSeq}` : ""}`;
}
...
export async function flushOutbox(
  ...
  for (const entry of await outbox.due(now)) {
    let ok = false;
    try {
      ok = await deliver({ ...entry.event, eventAt: entry.event.eventAt ?? entry.createdAt }, entry.id);
    } catch {
      ok = false;
    }
    if (ok) {
